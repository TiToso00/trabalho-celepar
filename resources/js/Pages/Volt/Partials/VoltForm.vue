<template>
  ...
  <div class="mb-3">
    <TextInput id="apelido" v-model="form.apelido" :error="form.errors.apelido" label="Apelido"></TextInput>
  </div>
  ...
  <div class="mb-3">
    ...
    <div v-if="medias">
      <div v-for="media in medias" class="d-flex gap-1">
        {{ media.file_name }}
        <Link :href="route('medias.destroy', media)" as="button" method="delete" type="button">X</Link>
      </div>
    </div>

    <div :class="{'is-invalid': form.errors.anexos}">
      <FilePondInput
        v-model="form.anexos"
        :reset="reset"
